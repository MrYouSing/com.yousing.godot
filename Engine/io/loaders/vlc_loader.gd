...
static func init()->void:
	...
	s_is_inited=true
	if !ClassDB.class_exists(&"VLCMedia"):return
	...
	for it in l.extensions:
		e="."+it;if !AudioLoader.s_pools.has(e) and !VideoLoader.s_pools.has(e):
...
static func load_from_file(f:String,c:bool=true)->Resource:
	if !FileAccess.file_exists(f):return create_from_url(f)
	if !s_is_inited:init()
