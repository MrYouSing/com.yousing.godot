...
const k_modifiers:Array[Vector2i]=[
	Vector2i(KEY_MASK_SHIFT,KEY_SHIFT),
	Vector2i(KEY_MASK_ALT,KEY_ALT),
	Vector2i(KEY_MASK_META,KEY_META),
	Vector2i(KEY_MASK_CTRL,KEY_CTRL),
]
...
func reload()->void:
	...
	if !path.is_empty():
		...
		var n:int=t.size();if n>1:
			...
			for i in n:
				l=t[1+n];k=LangExtension.str_to_enum(l[0],k_keys)
				src_keys[i]=k
				if 1<l.size():k=LangExtension.str_to_enum(l[1],k_keys)
				dst_keys[i]=k
	#
	if src_keys.size()!=dst_keys.size():
		dst_keys.assign(src_keys)
	#
	old_keys.clear();new_keys.clear()
...
func modifier(m:int)->int:
	...
	if n!=_modifier.x:
		_modifier.x=n
		#
		n=0;for it in k_modifiers:if on(it.y):n|=it.x
		var a:bool=n&KEY_MASK_META!=0;var b:bool=n&KEY_MASK_CTRL!=0
		if a or b:if a!=b:n|=KEY_MASK_CMD_OR_CTRL
		_modifier.y=n
	else:
		n=_modifier.y
	#
	if m&KEY_MASK_CMD_OR_CTRL==0:
		n&=~KEY_MASK_CMD_OR_CTRL
	else:
		n&=~(KEY_MASK_META|KEY_MASK_CTRL)
	return n

func shortcut(k:Key)->bool:
	if k!=0:
		var m:int=k&KEY_MODIFIER_MASK
		var n:int=modifier(m)
		if m==n:return down(k&KEY_CODE_MASK)
	return false
